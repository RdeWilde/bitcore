Contributing to ioncore
=======

We're working hard to make *ioncore* the most powerful JavaScript library for working with bitcoin. Our goal is to have *ioncore* be a library that can be used by anyone interested in bitcoin, and to level expertise differences with great design and documentation.

## Community

If there are any questions, etc., please feel to ask in one of the community channels:

- https://labs.bitpay.com/c/ioncore (Support Forum)
- https://gitter.im/bitpay/ioncore (Development Chat)
...
These are some global design goals in ioncore that any change must adhere.
...
When possible, ioncore objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of ioncore methods. For example, calling `JSON.stringify` on an ioncore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'ioncore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the ioncore object.
...
```sh
git remote add ioncore git@github.com:RdeWilde/ioncore.git
git pull --rebase ioncore master
```
...
```
Finally go to [github.com/rdewilde/ioncore](https://github.com/rdewilde/ioncore) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of ioncore.

If you have any questions feel free to post them to
[github.com/rdewilde/ioncore/issues](https://github.com/rdewilde/ioncore/issues).
